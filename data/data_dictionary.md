...
|  |  |  |  |  |  |  |  |  |
|  | `pop_density` |  | double |  | people/km² |  | Mean population density within the cell (GPWv4 rev. 11, SEDAC/CIESIN, 2020 estimate). |  |
|  |  |  |  |  |  |  |  |  |
...
-   **Covariates are spatial, not temporal.** Radiance is averaged across years; land cover and population density are from a single snapshot (NLCD 2019, GPWv4 2020). This reflects the finding that spatial variation dominates temporal variation at this scale.
...
The final dataset is produced by `R/06_build_grid_model_dataset.R`, which joins:

-   

-   `data/processed/gbif/grid_presence_10km.csv` (from `R/05_build_grid_presence.R`)

-   

-   `data/processed/covariates_grid/grid_covariates_10km.csv` (from `R/04_build_grid_covariates.R`)

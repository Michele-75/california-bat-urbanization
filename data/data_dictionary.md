...
This dataset is a balanced panel of 10 km grid cells × 3 bat species, covering the accessible modeling domain within California. Each row corresponds to one species in one grid cell, with cumulative presence/absence across 2012–2024 and grid-level landscape covariates.
...
| `is_present` | integer (0/1) | — | Binary presence indicator: 1 if `n_obs > 0`, 0 otherwise. This is the primary response variable for logistic modeling. |
...
| `pop_density` | double | people/km² | Mean population density within the cell (GPWv4 rev. 11, 2020 estimate, from SEDAC/CIESIN). |
| `log1p_pop_density` | double | log(people/km² + 1) | Log-transformed population density: `log(1 + pop_density)`. This is the primary predictor in the final models. |
...
-   **Covariates are spatial, not temporal.** Radiance is averaged across years; land cover and population density come from single snapshots (NLCD 2019, GPWv4 2020). This reflects the finding that spatial variation dominates temporal variation at this scale.
...
-   **Zero observations do not necessarily mean absence.** Most grid cells have never been surveyed for bats. The accessible domain restriction helps with this but does not eliminate it.
...
The final dataset is produced by `preprocessing_scripts/06_build_grid_model_dataset.R`, which joins:

-   `data/processed/gbif/grid_presence_10km.csv` (from `preprocessing_scripts/05_build_grid_presence.R`)
-   `data/processed/covariates_grid/grid_covariates_10km.csv` (from `preprocessing_scripts/04_build_grid_covariates.R`)

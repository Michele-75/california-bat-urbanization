...
TIGER/Line database via the `tigris` package
(`R/02_get_ca_boundary.R`), is used for spatially filtering bat points
and constructing the analysis grid.
...
-   **NLCD percent developed land** — the National Land Cover Database
    (2019, 30 m resolution) classifies pixels into land cover types.
    The proportion of pixels in the four "Developed" classes (Open Space
...
A fourth covariate — percent protected land from PAD-US 4.1 (GAP
Status 1–3) — was computed but excluded from the final modeling because
it added minimal predictive value once the urban variables were
included. The processing code is preserved in
...
year-level modeling. Aggregating across the full study period (2012–2024)
pools all available data into a single spatial snapshot, maximizing
statistical power at the cost of collapsing temporal dynamics — a
tradeoff revisited in the limitations section.

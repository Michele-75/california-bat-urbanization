...
`preprocessing_scripts/06_build_grid_model_dataset.R`) that can be run
in sequence to reproduce the full workflow from raw downloads to the
final analysis-ready dataset. This section walks through each stage in
the order the data were actually acquired and processed, highlighting
key spatial operations and design decisions along the way.
...
workflow in `preprocessing_scripts/01_get_clean_bat_points.R` begins by
resolving each focal species name against the GBIF backbone taxonomy
using `name_backbone()`, which returns a unique `species_key` — a stable
numeric identifier that avoids issues with taxonomic synonyms.
...
(`preprocessing_scripts/02_get_ca_boundary.R`), is used for spatially
filtering bat points and constructing the analysis grid.
